...
STATUS_PARA_SITUACAO = {"ativo": "ativo", "inativo": "bloqueado"}
SITUACAO_PARA_STATUS = {"ativo": "ativo", "bloqueado": "inativo"}
...
def listar_usuarios(
    ...
    try:
        ...
        emails_por_id = {
            ...
            for usuario in supabase_admin.auth.admin.list_users()
        }
...
def editar_usuario(usuario_id: UUID, dados: UsuarioEditar):
    try:
        ...
        if "cargo" in alteracoes:
            atualizacao["tipo_usuario"] = alteracoes["cargo"]
        if "status" in alteracoes:
            atualizacao["situacao"] = STATUS_PARA_SITUACAO.get(alteracoes["status"], alteracoes["status"])
        ...
        email = usuario_auth.user.email if usuario_auth and usuario_auth.user else ""
